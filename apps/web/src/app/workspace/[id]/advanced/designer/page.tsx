...
import { useAdvancedExecutionStore } from "../../../../../features/advanced/stores/execution.store"
...
export default function AdvancedDesignerPage() {
  ...
  const { logs, addLog } = useAdvancedExecutionStore()
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="space-y-2">
                {nodeLibrary.filter(n => n.type === "TRIGGER").map((node, i) => (
                  <div
                    key={i}
                    draggable
                    className="border bg-muted/40 hover:bg-muted p-2 rounded text-xs font-medium cursor-grab"
                  >
                    {node.label}
                  </div>
                ))}
        ...
        <div className="col-span-6 border bg-card rounded-xl relative flex items-center justify-center overflow-hidden bg-slate-950 text-white">
          ...
          <div className="z-10 text-center space-y-4">
            ...
            <div className="flex justify-center space-x-4">
              <button
                onClick={() => handleSelectNode("node-1")}
                className="px-3 py-2 border rounded-lg bg-slate-900 text-xs font-semibold hover:bg-slate-800"
              >
                Mock Trigger Node
              </button>
              <button
                onClick={() => handleSelectNode("node-2")}
                className="px-3 py-2 border rounded-lg bg-slate-900 text-xs font-semibold hover:bg-slate-800"
              >
                Mock Action Node
              </button>
