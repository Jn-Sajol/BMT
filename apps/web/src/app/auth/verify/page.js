...
const schema = zod_2.z.object({
    token: zod_2.z.string().min(1, "Verification token is required"),
});
function VerifyPage() {
    ...
    const onSubmit = async (data) => {
        try {
            setError(null);
            await auth_service_1.AuthService.verifyEmail(data);
            setSuccess(true);
            setTimeout(() => {
    ...
    return (<div className="space-y-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Confirm your corporate subscription</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <input type="text" {...register("token")} className="mt-1 block w-full rounded border px-3 py-2 text-sm bg-background"/>
