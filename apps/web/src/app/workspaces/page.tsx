...
export default function WorkspacesPage() {
  ...
  const { selectWorkspace, selectMode } = useWorkspace()
      ...
      <div className="w-full max-w-4xl space-y-8">
        ...
        <div className="bg-card border rounded-xl p-6 shadow-sm">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {mockWorkspaces.map((ws) => (
              <button
                key={ws.id}
                onClick={() => setSelectedWorkspaceState(ws)}
        ...
        <div className={`grid gap-6 md:grid-cols-2 transition-opacity ${selectedWorkspace ? "opacity-100" : "opacity-40 pointer-events-none"}`}>
          ...
          <div className="border bg-card rounded-xl p-6 flex flex-col justify-between shadow-sm hover:shadow-md transition">
            ...
            <button
              onClick={() => handleModeSelection("SAFE")}
              className="mt-8 w-full rounded-lg bg-blue-600 hover:bg-blue-700 py-2.5 text-sm font-semibold text-white transition"
            >
              Enter SAFE Gateway
            </button>
          </div>
          ...
          <div className="border bg-card rounded-xl p-6 flex flex-col justify-between shadow-sm hover:shadow-md transition">
            ...
            <button
              ...
              className="mt-8 w-full rounded-lg bg-orange-600 hover:bg-orange-700 py-2.5 text-sm font-semibold text-white transition"
            >
