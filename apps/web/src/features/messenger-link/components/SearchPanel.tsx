...
export function SearchPanel({ onSearch }: Props) {
  ...
  return (
    <div style={{ color: "#fff", fontFamily: "sans-serif", marginTop: "20px", background: "rgba(255,255,255,0.02)", padding: "16px", borderRadius: "8px" }}>
      <h4 style={{ color: "#60a5fa", margin: "0 0 12px 0" }}>Discover Public Messenger Invite Links</h4>
      <form onSubmit={handleTrigger} style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
        <input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="Keywords (e.g. SaaS, eCommerce)..." style={{ background: "#2d3748", border: "none", color: "#fff", padding: "8px", borderRadius: "6px", flex: 1 }} />
        ...
        <button type="submit" style={{ background: "#3b82f6", border: "none", color: "#fff", padding: "8px 16px", borderRadius: "6px", cursor: "pointer", fontWeight: "bold" }}>Find Invites</button>
      </form>
    </div>
  )
}
