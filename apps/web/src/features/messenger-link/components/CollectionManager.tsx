...
export function CollectionManager({ collections, onAdded, onDelete }: Props) {
  ...
  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault()
    fetch("/api/messenger-link/collections", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: colName }),
    }).then(() => {
      alert("Custom links collection directory folder created!")
      setColName("")
      onAdded()
    })
  }
  ...
  return (
    <div style={{ color: "#fff", fontFamily: "sans-serif", marginTop: "20px" }}>
      ...
      <form onSubmit={handleCreate} style={{ display: "flex", gap: "10px", marginBottom: "12px" }}>
        <input value={colName} onChange={(e) => setColName(e.target.value)} placeholder="Folder name (e.g. Sales channels)" style={{ background: "#2d3748", border: "none", color: "#fff", padding: "8px", borderRadius: "6px" }} />
