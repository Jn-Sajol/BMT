...
export function ReplyEditor({ inboxId, originalComment, initialReply, onReplied }: Props) {
  ...
  const handlePost = (e: React.FormEvent) => {
    e.preventDefault()
    fetch("/api/messenger-controller/reply", {
      method: "POST",
      ...
      .then((res) => res.json())
      .then(() => {
        alert("Reply manually approved and sent to sender!")
        onReplied()
      })
  }
  ...
  return (
    <div style={{ color: "#fff", fontFamily: "sans-serif", marginTop: "20px" }}>
      ...
      <form onSubmit={handlePost} style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        ...
        />
        <button type="submit" style={{ background: "#10b981", border: "none", color: "#fff", padding: "8px 16px", borderRadius: "6px", cursor: "pointer", fontWeight: "bold", maxWidth: "200px" }}>Approve & Send message</button>
      </form>
    </div>
  )
}
