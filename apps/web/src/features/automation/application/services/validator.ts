...
import { NodeTypeRegistry } from "../../domain/registries/node-type.registry"
...
export class WorkflowValidator {
  public static validate(nodes: WorkflowNode[], edges: WorkflowEdge[]): ValidationError[] {
    ...
    nodes.forEach((node) => {
      if (!NodeTypeRegistry.isSupported(node.type)) {
        errors.push({
  ...
  private static detectCycleKahn(nodes: WorkflowNode[], edges: WorkflowEdge[]): boolean {
