...
export function ReplyTemplateEditor({ onAdded }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    fetch("/api/comment-reply/templates", {
      method: "POST",
      ...
      .then((res) => res.json())
      .then(() => {
        alert("Reply template saved to library!")
        setContent("")
        setCategory("")
        setTags("")
        onAdded()
  ...
  return (
    <div style={{ color: "#fff", fontFamily: "sans-serif", marginTop: "20px" }}>
      <h4 style={{ color: "#60a5fa", margin: "0 0 10px 0" }}>Add Saved Reply Template</h4>
