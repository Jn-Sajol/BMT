...
export function ReplyLibrary({ templates, onDelete }: Props) {
  return (
    <div style={{ color: "#fff", fontFamily: "sans-serif", marginTop: "20px" }}>
      <h4 style={{ color: "#60a5fa", margin: "0 0 10px 0" }}>Saved Replies Library</h4>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "10px" }}>
        {templates.map((t) => (
          <div key={t.id} style={{ background: "rgba(255,255,255,0.03)", padding: "12px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.05)" }}>
            ...
            <div style={{ display: "flex", gap: "4px", marginTop: "6px", flexWrap: "wrap" }}>
              {t.tags.map((tag, idx) => (
                <span key={idx} style={{ fontSize: "9px", background: "rgba(255,255,255,0.1)", padding: "2px 6px", borderRadius: "10px" }}>#{tag}</span>
              ))}
            </div>
            <button
        ...
        {templates.length === 0 && <div style={{ color: "#9ca3af", fontSize: "13px" }}>No reply templates configured.</div>}
      </div>
    </div>
  )
}
