...
let MetaInsightsService = (() => {
    ...
    var MetaInsightsService = class {
        ...
        static {
            ...
            MetaInsightsService = _classThis = _classDescriptor.value;
        ...
        adSetInsightRepo;
        adInsightRepo;
        constructor(campaignInsightRepo, adSetInsightRepo, adInsightRepo) {
            this.campaignInsightRepo = campaignInsightRepo;
            this.adSetInsightRepo = adSetInsightRepo;
            this.adInsightRepo = adInsightRepo;
        }
        async getCampaignInsights(workspaceId) {
            const list = await this.campaignInsightRepo.findByWorkspaceId(workspaceId);
            return list.map(meta_insights_mapper_1.MetaInsightsMapper.toCampaignResponse);
        }
        async getAdSetInsights(workspaceId) {
            const list = await this.adSetInsightRepo.findByWorkspaceId(workspaceId);
            return list.map(meta_insights_mapper_1.MetaInsightsMapper.toAdSetResponse);
        }
        async getAdInsights(workspaceId) {
            const list = await this.adInsightRepo.findByWorkspaceId(workspaceId);
            return list.map(meta_insights_mapper_1.MetaInsightsMapper.toAdResponse);
        }
    };
    return MetaInsightsService = _classThis;
})();
