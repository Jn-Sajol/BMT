...
let DistributedLockService = (() => {
    ...
    let _classThis;
    var DistributedLockService = class {
        ...
        async acquireLock(scheduleId, nodeId, leaseMs) {
            const now = new Date();
        ...
        async renewLock(scheduleId, nodeId, leaseMs) {
            const now = new Date();
            const expiresAt = new Date(now.getTime() + leaseMs);
            try {
                const result = await this.prisma.automationScheduleLock.updateMany({
        ...
        async releaseLock(scheduleId, nodeId) {
            try {
