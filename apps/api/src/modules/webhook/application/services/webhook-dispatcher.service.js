...
let WebhookDispatcher = (() => {
    ...
    var WebhookDispatcher = class {
        ...
        static {
            ...
            WebhookDispatcher = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        inboxRepo;
        handlers;
        constructor(inboxRepo, handlers = []) {
            this.inboxRepo = inboxRepo;
            this.handlers = handlers;
        }
        async dispatch(provider, externalId, payload) {
            const idempotencyKey = `${provider}:${externalId}`;
            const duplicate = await this.inboxRepo.findByKey(idempotencyKey);
            if (duplicate) {
                return;
            }
            const inboxEntry = await this.inboxRepo.insertEvent(provider, externalId, idempotencyKey, payload);
            this.processEventAsync(inboxEntry.id, provider, payload);
        }
        async processEventAsync(jobId, provider, payload) {
            try {
        ...
        extractEventType(provider, payload) {
            if (provider === 'meta') {
    ...
    return WebhookDispatcher = _classThis;
})();
